...
export function outputTitle() {
  ...
  for (let index = 0; index < 4; index++) {
    ...
    switch (index) {
      case 0: cellElTitle.textContent = 'id';
      break;
      case 1: cellElTitle.textContent = 'title';
      break;
      case 2: cellElTitle.textContent = 'year';
      break;
      case 3: cellElTitle.textContent = 'imdb';
      break;
    };
...
export function outputFilms2 (arrayFilms) {
  for (const film of arrayFilms) {
    ...
    for (let index = 0; index < 4; index++) {
      let cellEl = document.createElement('td');
      switch (index) {
        case 0: cellEl.textContent = film.dataset.id;
        rowEl.setAttribute('data-id', film.dataset.id);
        break;
        case 1: cellEl.textContent = film.dataset.title;
        rowEl.setAttribute('data-title', film.dataset.title);
        break;
        case 2: cellEl.textContent = film.dataset.year;
        rowEl.setAttribute('data-year', film.dataset.year);
        break;
        case 3: cellEl.textContent = film.dataset.imdb;
...
export function sortFilm  (titleTable, arrayFilms, f) {
  if (isNaN(arrayFilms[0].dataset[titleTable])) {
    if (f === 1) {
      arrayFilms.sort((a,b) => a.dataset[titleTable].localeCompare(b.dataset[titleTable]));
    } else {
      arrayFilms.sort((a,b) => b.dataset[titleTable].localeCompare(a.dataset[titleTable]));
    }
  } else {
    if (f === 1) {
      arrayFilms.sort((a,b) => Number(a.dataset[titleTable]) - Number(b.dataset[titleTable]));
    } else {
      arrayFilms.sort((a,b) => Number(b.dataset[titleTable]) - Number(a.dataset[titleTable]));
    }  
  }
  return arrayFilms;
}
...
let filmsFromDom = document.querySelectorAll('tr');
